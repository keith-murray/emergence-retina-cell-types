...
from support import environment,optimize_func,make_output_test_video,stimuli,make_test, create_summary_prior, create_summary_apriori
...
class temp_conv(nn.Module):
    ...
    def forward(self, x):
        ...
        self.alpha[4]*torch.ones(101,device=device).float()
        kernel = torch.reshape(torch.flip(ker,(0,)), (1,1,101,1,1)).to(device)
        if self.TFbias:
...
class bipolar_pool(nn.Module):
    ...
    def forward(self, x):
        kernel = torch.relu(torch.reshape(F.relu(self.alpha), (1,1,1,4,4)).to(device))
...
class Retinal_NET(nn.Module):
    def __init__(self):
        ...
        self.space_conv = biploar_space((20,1,299,100,100))
        ...
        self.last_lay = bipolar_pool((20,1,299,20,20))
    ...
    def forward(self, x):
        ...
        cell_gang = torch.unsqueeze(torch.roll(torch.flatten(out,start_dim=3), -6, -1), -1)
...
if __name__ == "__main__":
    ...
    scene, cent_loc = environment(20, 5)
    torch.autograd.set_detect_anomaly(True)
    loss_vals, loss, pred_py, net = optimize_func(scene, cent_loc, net, 200)
    plt.plot(loss_vals)
    
    scene, cent_loc = environment(20, 5)
    loss_vals1, loss1, pred_py1, net = optimize_func(scene, cent_loc, net, 200)
    plt.plot(loss_vals1)
    
    scenef, cent_locf = stimuli(20, 5, 0, 0, 2, 9)
    make_test(net, scenef, cent_locf)
